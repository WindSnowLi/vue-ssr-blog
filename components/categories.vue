          ...
          <div class="row">
              ...
              <div class="col-md-6" :value="label" :key="i" v-for="(label,i) in labels">
                  <div class="my-post-category">
                      <nuxt-link :to="{ name:'article-type-id',params:{ id: label.id } }" class="d-flex align-items-center justify-content-center"
...
<script>
  import { getAllTypeByUserId } from '@/api/article.js'
  ...
  export default{
    ...
    methods: {
        getPageTitle () {
            return [
                { title: '首页', to: { name: 'index' } },
                { title: '分类', to: { name: 'article-type' } }
            ]
        }
    },
    created: function () {
